...
const GET = async () => {
  ...
  try {
    const { data } = await http.get(`/branches${usp.value ? `?${usp.value}` : ''}`);
    ...
    <div class="col-12">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb my-0">
          ...
          <li class="breadcrumb-item active"><span>Extensões</span></li>
        </ol>
      </nav>
    </div>
  </div>
      ...
      <div class="card mb-4">
        <div class="card-header">
          <div class="d-flex justify-content-between align-items-center">
            <div class="">
              Extensões (
              {{ branches.length }}/{{ meta?.per_page || 0 }} - {{ meta?.total || 0 }} )
            </div>
            ...
            <table class="table border mb-0">
              <thead class="fw-semibold text-nowrap">
                <tr class="align-middle">
                  ...
                  <th class="bg-body-secondary">Endereço</th>
                </tr>
              </thead>

              <tbody>
                <tr class="align-middle" v-bind:key="branch.id" v-for="branch in branches">
                  <td>{{ branch.id }}</td>
                  <td>{{ branch.name }}</td>
        ...
        <div class="card-footer" v-show="meta?.last_page !== 1">
          <div class="btn-group" role="group">
            ...
            <a
              v-bind:href="`/branches?page=${(meta?.current_page || 0) - 1}`"
            ...
            <a
              v-bind:href="`/branches?page=${meta?.last_page || 1}`"
